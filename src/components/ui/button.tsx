  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        icon: "bg-transparent hover:bg-transparent border-none shadow-none text-primary !p-0 !h-fit",
      },
      size: {
...
const TileButton = (props: React.ComponentProps<"button">) => {
  return (
    <button
      {...props}
      className={cn(
        "group items-start text-start rounded-xl bg-card border-border border-[1px] w-full p-6 flex gap-2 flex-col cursor-pointer hover:shadow-sm transition-all duration-200 ease-in-out justify-center disabled:opacity-60 disabled:pointer-events-none disabled:hover:shadow-none",
        props.className
      )}
    >
      {props.children}
...
export { Button, TileButton, buttonVariants };
